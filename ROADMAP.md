...
- Документация: README.md (EN) + README.ru.md (RU), взаимные ссылки; финальная семантика place-режима задана пользователем (мастер-кнопка, Alt-снап, G-оператор, ручное вкл/выкл)
...
### v3.2 — Быстрый turntable → ВЫНЕСЕН в отдельный аддон (решение 2026-08-29)
Турнтейбл — не свет, это камера-система. Детальный дизайн (панель, риг
pivot+Track To, линейные кейфреймы, Render Turntable) перенесён в план
нового аддона; в LAMPOCHKA не реализуется. Нумерация LAMPOCHKA дальше
без пропусков.

### v3.3 — Надёжность библиотек ✅ (сделано 2026-09-13, дистрибутив out/v3.3.0/)
- Авто-ремонт: мёртвая папка сцены при загрузке заменяется живой из prefs,
  затем из bundled-библиотеки team-сборки (единый `_resolve_scene_folders`
  для всех четырёх библиотек; живая папка сцены всегда сильнее prefs)
- Ремонт срабатывает и на правку prefs в живой сессии (update-колбэки)
- Индикация «Folder not found: <имя>» (ERROR) вместо безликого
  «Pick a folder» для мёртвого пути — во всех четырёх браузерах
- Мёртвый путь без живой замены остаётся (честная индикация), сидить
  мёртвый путь из prefs в пустое поле больше нельзя
- Мок-тест 205 проверок

### v3.4.0 — Пресеты: blend/zip-движок ✅ (сделано 2026-09-22, релиз 2026-09-22)
- JSON-движок заменён полностью: конвертация теряла empty-иерархии (родительские
  координаты уезжали до 31 м), нодовые цвета (35/42 светов) и сами иерархии
- Пакеты = zip → подпапка пресетов (PLS-лейаут `library/` флаттенится, zip-slip guard);
  setup = именованная коллекция package-.blend; Apply = append коллекции
  (bpy.data.libraries.load) — всё приезжает 1:1 (live-тест сверяет с прямой загрузкой)
- Карусель как у HDRI-браузера: карточки prev/active/next + ◀▶, переключение
  применяет сразу (update-колбек selected_preset)
- Save Setup — libraries.write в .blend (света + родительские empty + поддеревья)
- Список светов — фиксированной высоты (template_list, 8 строк, скролл): панель
  больше не меняет размер при переключении пресетов
- Очистка пакетов — в Preferences (Installed Preset Packages), не в сайдбаре
- Шапка панели с версией (LAMPOCHKA v3.4.0), как у STUKACH
- Legacy (Blender 3.6) ЗАКРЫТ решением юзера: extension-only, 4.2+

### v3.5 — Sun-система (бэклог v2.5, номер свободен)
